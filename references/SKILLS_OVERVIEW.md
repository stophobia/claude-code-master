# Anthropic Skills 전체 가이드

> 총 16개 스킬에 대한 기능 분석 및 설명 문서

---

## 목차

1. [PDF](#1-pdf) - PDF 파일 처리
2. [DOCX](#2-docx) - Word 문서 처리
3. [XLSX](#3-xlsx) - 스프레드시트 처리
4. [PPTX](#4-pptx) - 프레젠테이션 처리
5. [Frontend Design](#5-frontend-design) - 프론트엔드 UI 디자인
6. [Canvas Design](#6-canvas-design) - 시각 아트/포스터 제작
7. [Algorithmic Art](#7-algorithmic-art) - 제너레이티브 아트
8. [Web Artifacts Builder](#8-web-artifacts-builder) - Claude 아티팩트 빌더
9. [Theme Factory](#9-theme-factory) - 테마/스타일링
10. [Brand Guidelines](#10-brand-guidelines) - Anthropic 브랜드 가이드
11. [Doc Co-Authoring](#11-doc-co-authoring) - 문서 공동 작성
12. [Internal Comms](#12-internal-comms) - 사내 커뮤니케이션
13. [Skill Creator](#13-skill-creator) - 스킬 생성 도구
14. [MCP Builder](#14-mcp-builder) - MCP 서버 개발
15. [Webapp Testing](#15-webapp-testing) - 웹앱 테스팅
16. [Slack GIF Creator](#16-slack-gif-creator) - Slack용 GIF 제작
...
### 개요
PDF 파일과 관련된 모든 작업을 처리하는 종합 스킬. 읽기, 생성, 편집, 병합, 분할, 폼 작성, 암호화 등 PDF 관련 전 기능을 지원한다.
...
| 폼 작성 | 입력 가능/불가능 폼 자동 입력 | pypdf, pdf-lib, 전용 스크립트 |
...
### 개요
Word 문서(.docx) 생성, 읽기, 편집, 분석을 위한 스킬. XML 기반의 정밀한 편집과 변경 추적(Tracked Changes), 코멘트 기능을 지원한다.
...
### 개요
스프레드시트 파일(.xlsx, .xlsm, .csv, .tsv) 생성, 편집, 분석을 위한 스킬. 재무 모델링, 데이터 분석, 차트 생성 등을 지원하며, Excel 수식 사용을 강력히 권장한다.
...
### 개요
PowerPoint 프레젠테이션(.pptx) 생성, 읽기, 편집을 위한 스킬. 디자인 가이드라인, 색상 팔레트, 타이포그래피, 레이아웃 권장사항을 포함한다.
...
### 디자인 가이드라인
- **색상 팔레트**: 10가지 프리셋 테마 (Midnight Executive, Forest & Moss, Coral Energy 등)
- **타이포그래피**: 헤더 36-44pt, 본문 14-16pt, 캡션 10-12pt
- **레이아웃**: 2컬럼, 아이콘+텍스트, 그리드, 하프블리드 이미지
- **금지 사항**: 동일 레이아웃 반복, 텍스트만 있는 슬라이드, 제목 아래 액센트 라인
...
### 개요
독창적이고 프로덕션 수준의 프론트엔드 인터페이스를 만드는 스킬. "AI 슬롭" 미학을 피하고, 대담하고 기억에 남는 UI를 생성한다.
...
**경로**: `skills/canvas-design/`

### 개요
디자인 철학 기반의 시각 아트를 .png/.pdf로 생성하는 스킬. 포스터, 아트 작품, 디자인 등 정적인 비주얼 제작에 특화되어 있다.
...
**경로**: `skills/algorithmic-art/`

### 개요
p5.js를 사용한 제너레이티브/알고리즘 아트 생성 스킬. 시드 기반 랜덤성과 인터랙티브 파라미터 탐색을 지원한다.
...
**경로**: `skills/web-artifacts-builder/`

### 개요
Claude.ai용 복잡한 HTML 아티팩트를 React + TypeScript + Tailwind CSS + shadcn/ui로 빌드하는 스킬.
...
**경로**: `skills/theme-factory/`

### 개요
슬라이드, 문서, 보고서, 랜딩 페이지 등에 적용할 수 있는 10가지 프리셋 테마를 제공하는 스타일링 스킬.
...
**경로**: `skills/brand-guidelines/`

### 개요
Anthropic의 공식 브랜드 색상과 타이포그래피를 아티팩트에 적용하는 스킬.
...
**경로**: `skills/doc-coauthoring/`

### 개요
문서를 공동 작성하기 위한 구조화된 3단계 워크플로우를 제공하는 스킬. 기술 스펙, 제안서, 의사결정 문서 등에 활용한다.
...
**경로**: `skills/internal-comms/`

### 개요
사내 커뮤니케이션 작성을 위한 가이드라인과 템플릿을 제공하는 스킬.
...
### 개요
새로운 스킬을 생성하거나 기존 스킬을 업데이트하기 위한 가이드 스킬. 스킬의 구조, 설계 원칙, 생성 프로세스를 안내한다.
...
**경로**: `skills/mcp-builder/`

### 개요
LLM이 외부 서비스와 상호작용할 수 있는 MCP(Model Context Protocol) 서버를 구축하기 위한 가이드 스킬.
...
### 지원 언어
- **TypeScript (권장)**: Zod 스키마, `server.registerTool`
- **Python**: Pydantic, FastMCP `@mcp.tool`

### 참조 문서
- `reference/mcp_best_practices.md` - MCP 핵심 가이드라인
- `reference/node_mcp_server.md` - TypeScript 구현 가이드
- `reference/python_mcp_server.md` - Python 구현 가이드
- `reference/evaluation.md` - 평가 생성 가이드
...
### 개요
Playwright를 사용하여 로컬 웹 앱을 테스트하는 스킬. UI 기능 검증, 디버깅, 스크린샷 캡처, 브라우저 로그 확인을 지원한다.
...
**경로**: `skills/slack-gif-creator/`

### 개요
Slack에 최적화된 애니메이션 GIF를 제작하는 스킬. PIL 기반 그래픽, 이징 함수, 유효성 검사 도구를 제공한다.
...
| 스킬 | 산출물 |
|------|--------|
| Frontend Design | HTML/CSS/JS, React 컴포넌트 |
| Canvas Design | .png, .pdf (시각 아트) |
| Algorithmic Art | .html (p5.js 제너레이티브 아트) |
...
| Brand Guidelines | Anthropic 브랜드 스타일 적용 |

### 워크플로우/도구
| 스킬 | 용도 |
|------|------|
| Doc Co-Authoring | 구조화된 문서 공동 작성 |
| Internal Comms | 사내 커뮤니케이션 작성 |
| Skill Creator | 새 스킬 생성 가이드 |
| MCP Builder | MCP 서버 개발 가이드 |
| Webapp Testing | Playwright 기반 웹앱 테스팅 |
| Web Artifacts Builder | Claude.ai 아티팩트 빌드 |
